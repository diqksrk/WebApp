...
// test start
  ...
  $nowseason = "SELECT * FROM board b join board_detail bd on b.id = bd.id LIMIT $start";
...
<?php
	...
	if($block != 1){
?>
			<li><a href="http://localhost/newteample/all.php?page=1">[처음]</a></li>
			<li><a href="http://localhost/newteample/all.php?page=<?=$start_page-1?>">[이전]</a></li>
<?php
		}
...
?>
			<li><a href="http://localhost/newteample/all.php?page=<?=$i?>">[<?=$i?>]</a></li>
<?php
	}
	if($block != $max_block){
?>
 			<li><a href="http://localhost/newteample/all.php?page=<?=$end_page+1?>">[다음]</a></li>
 			<li><a href="http://localhost/newteample/all.php?page=<?=$total_pages?>">[마지막]</a></li>
<?php
	}
