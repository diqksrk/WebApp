...
// test start
  ...
  $user = "SELECT * FROM favorite WHERE id=\"$id\"";
  $userwant = $db->query($user);

  foreach($userwant as $wants){
    $want_i1 = $wants["item_1"];
    $want_i2 = $wants["item_2"];
    $want_i3 = $wants["item_3"];
  }

    $wanting = "";  // ??OO

    $wanting = "SELECT * FROM board b join board_detail bd on b.id = bd.id WHERE bd.favorite REGEXP '$want_i1$|$want_i2$|$want_i3$' LIMIT $start";

	  $results = $db->query("$wanting, $size");
...
<?php
	...
	if($block != 1){
?>
			<li><a href="http://localhost/newteample/wanting.php?page=1">[처음]</a></li>
			<li><a href="http://localhost/newteample/wanting.php?page=<?=$start_page-1?>">[이전]</a></li>
<?php
		}
...
?>
			<li><a href="http://localhost/newteample/wanting.php?page=<?=$i?>">[<?=$i?>]</a></li>
<?php
	}
	if($block != $max_block){
?>
 			<li><a href="http://localhost/newteample/wanting.php?page=<?=$end_page+1?>">[다음]</a></li>
 			<li><a href="http://localhost/newteample/wanting.php?page=<?=$total_pages?>">[마지막]</a></li>
<?php
	}
